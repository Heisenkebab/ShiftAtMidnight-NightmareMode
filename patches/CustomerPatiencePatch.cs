...
public static class CustomerPatiencePatch
{
    ...
    {
        if (!NightmareSettings.Enabled.Value || !NightmareSettings.ShouldLimitCustomerPatience.Value) return;

        if (__instance.patienceCanvas == null) return;

        if (!__instance.HasStateAuthority) return;

        Image patienceBar = __instance.patienceCanvas.GetComponentInChildren<Image>(true);
